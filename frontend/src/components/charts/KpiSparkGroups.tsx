...
export function KpiSparkGroups({ data, schema }: KpiSparkGroupsProps) {
    ...
    const rawMetrics = schema?.metrics || {};

    // Robustly infer derived metrics availability if base metrics are present
    // This handles cases where backend schema might only report raw columns
    const metrics = { ...rawMetrics };
    if (metrics.spend && metrics.impressions) metrics.cpm = true;
    if (metrics.clicks && metrics.impressions) metrics.ctr = true;
    if (metrics.spend && metrics.clicks) metrics.cpc = true;
    if (metrics.spend && metrics.conversions) metrics.cpa = true;
    if (metrics.spend && metrics.revenue) metrics.roas = true;
