...
class RegressionPipeline:
    ...
    def run(
        ...
        metrics = ModelEvaluator.evaluate(y_train, y_test, y_train_pred, y_test_pred)
        ...
        prediction_intervals = ModelEvaluator.generate_prediction_intervals(
            y_test_pred, metrics.residual_std
        )
    ...
    def _prepare_data(
        ...
        # One-hot encode dimensions if requested
        if encode_dimensions:
            for dim in encode_dimensions:
                if dim in df.columns:
                    dummies = pd.get_dummies(df[dim], prefix=dim, drop_first=True)
                    # Convert booleans to ints (0/1) for better compatibility
