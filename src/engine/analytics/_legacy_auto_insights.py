...
from src.core.utils.resilience import (
...
class MediaAnalyticsExpert:
    ...
    def analyze_all(self, df: Union[pd.DataFrame, pl.DataFrame], 
        ...
        Supports Polars for accelerated metric calculation.
        
        Args:
            df: DataFrame with campaign data (Pandas or Polars)
        ...
        # OPTIMIZATION: If Polars, calculate core metrics FAST before conversion
        if isinstance(df, pl.DataFrame):
            try:
                logger.info("⚡ Using Polars for fast metric aggregation")
                # Calculate metrics on the raw Polars DF
                metrics_data = self._calculate_metrics(df)
                
                # Convert to Pandas for downstream compatibility
                # Most analysis agents still require Pandas
                df = df.to_pandas()
            except Exception as e:
                logger.error(f"Polars optimization failed, falling back to Pandas: {e}")
                if isinstance(df, pl.DataFrame):
                    df = df.to_pandas()
                metrics_data = None
        ...
        # Ensure df is Pandas at this point
        if not isinstance(df, pd.DataFrame): 
             # Should be handled above, but just in case of weird types
             df = pd.DataFrame(df)
        ...
        # Only calculate if we didn't already via Polars
        if not metrics_data:
            metrics_data = self._calculate_metrics(df)
    ...
    def _calculate_metrics(self, df: Union[pd.DataFrame, pl.DataFrame]) -> Dict[str, Any]:
        """Calculate comprehensive metrics from the data (Optimized for Polars)."""
        metrics = {
            "overview": {},
            "by_campaign": {},
            "by_platform": {},
        ...
        # Determine if input is Polars
        is_polars = isinstance(df, pl.DataFrame)
        
        # Helper for Polars conditional aggregation
        def _get_pl_col(name: str):
            return pl.col(name) if name in df.columns else pl.lit(0)

        # Overview calculation
        if is_polars:
            # Polars Aggregation
            total_spend = float(df.select(pl.col('Spend').fill_null(0).sum()).item()) if 'Spend' in df.columns else 0
            total_conversions = float(df.select(pl.col('Conversions').fill_null(0).sum()).item()) if 'Conversions' in df.columns else 0
            total_impressions = float(df.select(pl.col('Impressions').fill_null(0).sum()).item()) if 'Impressions' in df.columns else 0
            total_clicks = float(df.select(pl.col('Clicks').fill_null(0).sum()).item()) if 'Clicks' in df.columns else 0
            
            campaign_count = df.select(pl.col('Campaign').n_unique()).item() if 'Campaign' in df.columns else 0
            platform_count = df.select(pl.col('Platform').n_unique()).item() if 'Platform' in df.columns else 0
            
            avg_roas = float(df.select(pl.col('ROAS').mean()).item()) if 'ROAS' in df.columns else 0
        else:
            # Fallback for Pandas (legacy support)
            return self._calculate_metrics_pandas(df)
        ...
        if 'Campaign' in df.columns:
            try:
                ...
                aggs = []
                if 'Spend' in df.columns: aggs.append(pl.col('Spend').sum())
                if 'Conversions' in df.columns: aggs.append(pl.col('Conversions').sum())
                if 'Impressions' in df.columns: aggs.append(pl.col('Impressions').sum())
                if 'Clicks' in df.columns: aggs.append(pl.col('Clicks').sum())
                if 'ROAS' in df.columns: aggs.append(pl.col('ROAS').mean())
                if 'CPA' in df.columns: aggs.append(pl.col('CPA').mean())
                
                camp_df = df.group_by('Campaign').agg(aggs)
                
                # Convert to dict {campaign_name: {metrics}} using to_pandas().to_dict('index') or struct
                # For compatibility with existing format: Dict[campaign_name, Dict[metric, val]]
                # Keeping conversion to pandas for easy dict structure creation for now
                metrics["by_campaign"] = camp_df.to_pandas().set_index('Campaign').to_dict('index')
            except Exception as e:
                logger.warning(f"Campaign aggregation failed: {e}")
        ...
        if 'Platform' in df.columns:
            try:
                ...
                plat_df = df.group_by('Platform').agg(aggs)
                metrics["by_platform"] = plat_df.to_pandas().set_index('Platform').to_dict('index')
            except Exception as e:
                logger.warning(f"Platform aggregation failed: {e}")
                
        return metrics

    def _calculate_metrics_pandas(self, df: pd.DataFrame) -> Dict[str, Any]:
        """Legacy Pandas implementation of metrics calculation."""
...
Then "DETAILED:" then the detailed summary."""
        ...
        from src.core.utils.summary_formatter import format_summary
        ...
        from src.core.utils.structure_enforcer import enforce_structure
        if detailed_summary:
...
Generate the complete BRIEF and DETAILED RAG-enhanced analysis now:"""
    ...
    def _generate_executive_summary_with_rag(self, 
        ...
        try:
            ...
            from src.core.utils.campaign_context import CampaignContextAnalyzer
            ...
            if self.use_anthropic and self.anthropic_api_key:
                try:
                    from src.core.utils.anthropic_helpers import call_anthropic_http
    ...
    def _score_recommendations(
        ...
        """
        from src.core.utils.confidence_scorer import ConfidenceScorer, assess_data_quality
        from src.core.utils.recommendation_validator import RecommendationValidator
