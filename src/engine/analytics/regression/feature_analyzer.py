...
class FeatureAnalyzer:
    ...
    def compute_vif(X: pd.DataFrame, threshold_moderate: float = 5.0, threshold_high: float = 10.0) -> Dict:
        ...
        # Overall assessment
        valid_vifs = [r.vif for r in vif_results if r.vif < 999]
        max_vif = max(valid_vifs) if valid_vifs else 999.9
        high_vif_count = sum(1 for r in vif_results if r.vif > threshold_high)
        
        if max_vif >= 999:
            overall_status = "Error"
            overall_message = "⚠️ Could not compute VIF (perfect multicollinearity or insufficient data)"
