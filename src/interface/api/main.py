...
async def lifespan(app: FastAPI):
    ...
    try:
        if hasattr(db_manager, 'shutdown'):
            db_manager.shutdown()
        logger.info("Lifespan: DB Connection Closed")
    except Exception as e:
        logger.error(f"Lifespan: DB Shutdown Failed: {e}")
...
logger.info("Skipped API Gateway (Safe Mode)")

# Setup exception handlers (MUST be before other middleware)
# setup_exception_handlers(app)
logger.info("Skipped Exception Handlers")
...
# logger.info("Skipped Router V1")

# Setup Prometheus metrics
# try:
#     from src.ops.monitoring.prometheus_metrics import setup_prometheus
#     setup_prometheus(app)
#     logger.info("✅ Prometheus metrics enabled at /metrics")
# except ImportError:
#     logger.warning("Prometheus metrics not available (install prometheus-client)")
logger.info("Skipped Prometheus (Safe Mode)")
...
if __name__ == "__main__":
    ...
    uvicorn.run(
        "src.interface.api.main:app" if os.getenv("PRODUCTION_MODE", "false").lower() == "false" else app,
        host=settings.api_host,
        port=settings.api_port,
        log_level="info",
        reload=os.getenv("PRODUCTION_MODE", "false").lower() == "false"
    )
