"""
Neo4j Loader for KG-RAG ETL

Batch loads transformed data into Neo4j using UNWIND for efficiency.
"""
...
class Neo4jLoader:
    """
    Load transformed data into Neo4j using batch UNWIND operations.
    ...
    def load_campaigns(
        ...
        Args:
            campaigns: List of transformed campaign records
            batch_size: Batch size for UNWIND
            
        Returns:
            Summary with nodes created/updated
        """
    ...
    def load_campaign_platform_relationships(
        ...
        Args:
            campaigns: Campaign records with platform_id
            
        Returns:
            Summary with relationships created
        """
        query = """
        UNWIND $batch AS row
        ...
        # Filter to campaigns with platform_id
    ...
    def load_targeting(
        ...
        """
        Load Targeting nodes and create relationships to Campaigns.
        
        Args:
            targeting_records: List of transformed targeting records
            batch_size: Batch size for UNWIND
    ...
    def load_metrics(
        ...
        """
        Load Metric nodes and create relationships to Campaigns.
        
        Args:
            metrics: List of transformed metric records
            batch_size: Batch size for UNWIND
        ...
        SET m.campaign_id = row.campaign_id,
            ...
            m.device_types = row.device_types,
            m.age_range = row.age_range,
            m.geo_countries = row.geo_countries,
    ...
    def update_campaign_totals(
        ...
        """
        Update campaign totals from metrics.
    ...
    def update_all_campaign_totals(self) -> int:
        """
        Update totals for all campaigns.
    ...
    def _load_batches(
        ...
        Args:
            ...
            node_type: Type name for logging
        ...
        for i in range(0, len(records), batch_size):
            ...
            logger.debug(
                f"Loaded {node_type} batch {i // batch_size + 1}: "
                f"{len(batch)} records"
            )
        
        logger.info(
            f"Loaded {len(records)} {node_type} records: "
            f"{total_created} created, {total_properties} properties set"
        )
